import React, { useState } from 'react'
import 'antd/dist/antd.css';
import { Button, Descriptions, Divider, Tag, Typography} from 'antd';
import { AppstoreOutlined, MailOutlined, SettingOutlined, UserOutlined, PoweroffOutlined, FrownOutlined, MehOutlined } from '@ant-design/icons';
import { Layout, Menu, Breadcrumb, Avatar, Card, Col, Row, Image, Collapse, Badge, Rate} from 'antd';
import { SmileOutlined } from '@ant-design/icons';
import '../css/UserDashboard.css';
//import logo from '../Assets/SmartLogo.png'
//import profimg from '../Assets/prof.png'
const { Header, Content, Footer, Sider } = Layout;
const{Title}=Typography;
const {SubMenu} = Menu;
var currentUser = 'Kkmehta12';
var first_name = 'Kunal'
var last_name = 'Mehta'
var dob = '01/12/1998'
var sex = 'M'
var subscription = 'Gold'
var linkedprovider = 'AHunt'
var providerfname = 'Alan'
var providerlname = 'Hunt'
var providerrating = 5;
var Ratetext = 'Rate ' + providerfname + ' ' + providerlname + '?';

const { Panel } = Collapse;
const customIcons = {
    1: <FrownOutlined />,
    2: <FrownOutlined />,
    3: <MehOutlined />,
    4: <SmileOutlined />,
    5: <SmileOutlined />,
  };

function UserDashboard()
{
    const [visible, setVisible] = useState(false);
    return(
        
        <Layout>
            <Layout padding='0px 0px 0px'>
                <Sider width={240} style={{background:"#A9A9A9", padding:"0px 0px 0px"}}>
                    <Menu mode="inline" defaultSelectedKeys={['1']} defaultOpenKeys={['sub1']} style={{height:"100%", borderRight:0}}>
                        <SubMenu key="sub1" title={<span><SmileOutlined/>My Information</span>}>
                            <Menu.Item key="1">Personal Details</Menu.Item>
                            <Menu.Item key="2">Preferences</Menu.Item>
                            <Menu.Item key="3">Subscription</Menu.Item>
                            <Menu.Item key="4">Newsletter</Menu.Item>
                            <Menu.Item key="5">Delete Account</Menu.Item>
                        </SubMenu>
                        <SubMenu key="sub2" title={<span><SmileOutlined/>Goals and Progress</span>}>
                            <Menu.Item key="5">Goal1</Menu.Item>
                            <Menu.Item key="6">Goal2</Menu.Item>
                            <Menu.Item key="7">Goal3</Menu.Item>
                            <Menu.Item key="8">Goal4</Menu.Item>
                        </SubMenu>
                        <SubMenu key="sub3" title={<span><SmileOutlined/>subnav 3</span>}>
                            <Menu.Item key="9">option9</Menu.Item>
                            <Menu.Item key="10">option10</Menu.Item>
                            <Menu.Item key="11">option11</Menu.Item>
                            <Menu.Item key="12">option12</Menu.Item>
                        </SubMenu>

                    </Menu>
                </Sider>
                <Layout style={{ padding: '24px 24px 24px', background:'darkgray' }}>
                    <Content style={{ background: 'white', padding: 24, margin: 0, minHeight: 280 }}>
                        <div className="site-card-wrapper">
                            <Row gutter = {16}>
                                <Card hoverable style={{ width: '45%'}}>
                                    <div style={{textAlign:'center'}}>
                                    <Image style={{width:'200px'}} src="https://cdna.artstation.com/p/assets/images/images/008/726/070/large/fadly-romdhani-arthas-menethil-v2-low-res-copy.jpg?1514905918" fallback="data:image/png;base64,"/>
                                    </div>
                                    <Descriptions bordered title = {currentUser} size="middle" layout="horizontal" column={1}>
                                        <Descriptions.Item label="First Name">{first_name}</Descriptions.Item>
                                        <Descriptions.Item label="Last Name">{last_name}</Descriptions.Item>
                                        <Descriptions.Item label="Date of Birth">{dob}</Descriptions.Item>
                                        <Descriptions.Item label="Sex">{sex}</Descriptions.Item>
                                        <Descriptions.Item label="Subscription"><Badge status="processing"/><Tag color={subscription}>{subscription}</Tag></Descriptions.Item>

                                    </Descriptions>
                                </Card>
                                <Col span = {12}>
                                <Row>
                                    <Card hoverable style={{ width: '100%', float:'right'}}>
                                        <text className="SurveysTitle">Surveys</text>
                                        <Collapse accordion>
                                            <Panel header="Explore Your Options" key="1">
                                            <p><text>Survey Desc from table</text></p>
                                            </Panel>
                                            <Panel header="What are you looking for?" key="2">
                                            <p><text>Survey Desc from table</text></p>
                                            </Panel>
                                            <Panel header="Need to talk? We're here" key="3">
                                            <p><text>Survey Desc from table</text></p>
                                            </Panel>
                                            <Panel header="Ready for therapy? Let's match you!" key="4">
                                            <p><text>Survey Desc from table</text></p>
                                            </Panel>
                                        </Collapse>
                                    </Card>
                                </Row>
                                <Row>
                                    <Card hoverable style={{ width: '100%', float:'right'}}>
                                        <Row>
                                            <Col span = {8}>
                                                <Image style={{width:"100%"}} src="https://i.pinimg.com/originals/a5/eb/bb/a5ebbb20ecdd5de25cf5828ab818b2b0.jpg" fallback="data:image/png;base64,"/>
                                                <text className="SurveysTitle" style={{textAlign:"center"}}>{linkedprovider}</text>
                                            </Col>
                                            <Col span = {14} offset = {2}>
                                            <Descriptions bordered size="middle" layout="horizontal" column={1}>
                                                <Descriptions.Item label="First Name">{providerfname}</Descriptions.Item>
                                                <Descriptions.Item label="Last Name">{providerlname}</Descriptions.Item>
                                                <Descriptions.Item label="Avg. Rating"><Rate disabled count={5} value={providerrating}/></Descriptions.Item>
                                            </Descriptions>
                                            <p></p>
                                            <Collapse accordion>
                                                <Panel header={Ratetext} key="1">
                                                    <p><text>Link to Survey</text></p>
                                                </Panel>
                                            </Collapse>
                                            </Col>
                                        </Row>
                                    </Card>
                                </Row>
                                </Col>
                            </Row>
                        </div>
                    </Content>
                </Layout>
            </Layout>
        </Layout>
    )
}
export default UserDashboard